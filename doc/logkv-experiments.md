...
2026-09-25に現行コードをパッケージへ整理しました。過去の記録内のパスは実行当時のものです。
再現には各記録のcommitまたは保存済みソースを使ってください。[現行配置との対応](repository-layout.md)。

2026-09-16、[固定減衰の5k LM対照訓練](logkv-lm-fixed-decay.md)は14:57 JSTに完了。
固定/学習可能lossは3.62275/3.61796、低温1024生成の強い反復は双方4/9。
圧縮L2以上のattentionは24.12%/28.17%。事前確認込み4.61時間、GPU解放・再監査済み。

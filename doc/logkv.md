...
- Copying/Selective の遠距離 plateau の要因分解（レベル減衰の有無 / 係数、訓練延長）。
  順序情報の保持強化（Selective の U 字型劣化・bag 最適への収束が示すとおり、圧縮を
  跨いだ順序符号が現状の主弱点）
- 同一トークン連続区間での位置縮退への対策（レベル位相 j = 位置の C 進数桁を有界・周期的な
  特徴として与える案など。固定ベクトルの蓄積を避ける方針との整合を要検討）

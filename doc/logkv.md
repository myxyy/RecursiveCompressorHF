...
- 域内改善と外挿維持を両立する最も均衡の取れた Selective プロファイル（ただし conv 版との
  差は依然大きい）
- **本番 LM への適用は要検証**: β<0 は §5.2 の多重計上増幅を再導入しうるため、長文生成での
  執着再発チェック（§6.6 と同じ手順）が必須。安全策として β≥0 制約（softplus 等）の
  オプションも検討余地

## 7. 未解決・今後の候補

- 学習可能減衰の本番 LM 検証（5000 步 + 1024/10k トークン生成の反復指標、学習された β の分布）

...
- **T=1 異常が Copying と完全同一署名で再現**（答え位置 7・8 のみ 0.60/0.51）。構造分析に
  よる有力仮説: T=1 では s=16..19 がレベル 1 位相同一（u=1, j=0）かつレベル 0 窓が全
  マーカーの縮退群になり、s=18/19 は窓内容の多重集合がほぼ区別不能になって「今何桁目か」
  が壊れる（s=20 は j=1 で位相が変わり生存）。T≥2 は答え位置が位相をまたぐため衝突しない
...
- Copying/Selective の遠距離 plateau の要因分解（レベル減衰の有無 / 係数、訓練延長）。
  ...
  跨いだ順序符号が現状の主弱点）
- T=1 縮退群仮説の直接検証（位相をずらした配置での再評価、窓内容の類似度測定）

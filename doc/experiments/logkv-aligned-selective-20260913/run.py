...
import sys
import time
from common import HERE, ROOT, SOURCE, COPY_ROOT, INITIAL, MODES, now, save, sha, run_dir
...
def main():
    ...
    assert preflight['task']=='selective-copying'
    ...
    assert json.loads(copy_review.read_text())['passed']
    manifest=json.loads((COPY_ROOT/'source_manifest.json').read_text())
    save(ROOT/'source_manifest.json',manifest)
    for f,h in manifest['files'].items(): assert sha(SOURCE/f)==h
    record=dict(task='selective-copying',copying_review_sha256=sha(copy_review),state='starting',started=now(),pid=os.getpid(),gpu_limit=2,gpus=[0,1],
        ...
        prerequisite_hashes={f:sha(HERE/f) for f in ['preflight.json','validation.json','evaluation_smoke.json']},
        initial_weights_sha256=sha(INITIAL),next_stage_queued=False)

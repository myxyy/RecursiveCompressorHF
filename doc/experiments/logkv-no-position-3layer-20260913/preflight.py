...
def main():
    ...
    config = LogKVConfig(vocab_size=10, d_model=512, num_heads=8, d_ff=1024,
        num_layers=NUM_LAYERS, chunk_size=4, phase_emb=False, phase_levels=2,
    ...
    assert extra and all(key.startswith('layers.2.') for key in extra)
    ...
    record = dict(passed=False, tasks=list(MODES), initial_weights_identical=True,
        num_params=NUM_PARAMS, num_layers=NUM_LAYERS, shared_two_layer_initial_weights_identical=True,
        base_initial_sha256=sha(BASE_INITIAL), extra_layer_parameters=sum(v.numel() for v in fresh_extra.values()), initial_weights_path=str(INITIAL),
    ...
    try:
        ...
        for mode in MODES:
            ...
            print(f'{mode}: 20-step task repeat bitexact across GPUs 2/3', flush=True)

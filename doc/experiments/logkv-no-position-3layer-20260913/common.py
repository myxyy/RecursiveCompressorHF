...
REPO = HERE.parents[2]
ROOT = Path('/mnt/raid0/RecursiveCompressor/experiments/logkv-no-position-3layer-20260913')
...
def name(mode):
    ...
    return 'no-position-3layer-fixed10-20260913'
...
def command(mode, python):
    return [python, str(HERE / 'train_entry.py'), '--run-name', name(mode),
        ...
        '--d-model', '512', '--num-heads', '8', '--d-ff', '1024', '--num-layers', str(NUM_LAYERS),

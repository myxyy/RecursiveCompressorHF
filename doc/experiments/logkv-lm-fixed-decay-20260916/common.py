...
REPO = HERE.parents[2]
ROOT = Path('/mnt/raid0/RecursiveCompressor/experiments/logkv-lm-fixed-decay-20260916')
SOURCE = ROOT / 'source'
DATA = ROOT / 'data'
RUN_NAME = 'd1024-h8-l16-conv4-fixed-decay-5000'
...
BASE = HERE.parent / "logkv-lm-learnable-decay-20260915"
BASE_ROOT = ROOT.parent / BASE.name

def shared_hash(model):
    h=hashlib.sha256()
    for name,tensor in sorted(model.state_dict().items()):
        if name.endswith(".level_decay"): continue

...
def main():
    ...
    record=dict(mode=a.mode,position_mode=a.mode,deterministic_training=True,started=now(),state='running',commands=[])
    ...
    try:
        ...
        assert json.loads((run_dir(a.mode)/'initialization_audit.json').read_text())['passed']

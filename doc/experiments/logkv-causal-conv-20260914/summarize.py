...
def main():
    ...
    def audit(mode,cp,kind,metrics,records,ts,n):
        ...
        for T in ts:
            ...
            rows.append(dict(mode=mode,task=mode,position_mode="none",num_layers=NUM_LAYERS, conv_kernel_size=4,checkpoint=cp,evaluation=kind,T=T,n=n,
    ...
    for mode in MODES:
        ...
        for cp,folder in [('best','model_best'),('final','model')]:
            ...
            assert not cfg['learnable_decay'] and not cfg['kv_norm'] and not cfg['level_amplify']
    ...
    table=['# Two-layer LogKV with causal convolution and no phase embeddings: automatically audited results','',

"""CPU paired prediction counts for baseline versus convolution on identical data."""
import json
...
def compare_records(two, three):
    ...
    for key in sorted(two, key=int):
        ...
        rows.append(dict(T=int(key), n=len(target),
            exact_baseline=sum(map(all, c2)), exact_conv=sum(map(all, c3)),
            ...
            digit_errors_conv=[sum(not c[i] for c in c3) for i in range(10)],
...
def main():
    ...
    save(dest / 'baseline_comparison.json', dict(state='complete', passed=True,
        ...
        limitations='One seed; convolution adds 6144 parameters. This is token-stream convolution, not per-compression-level convolution.'))

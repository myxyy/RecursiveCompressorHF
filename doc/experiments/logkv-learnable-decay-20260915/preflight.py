...
def main():
    ...
    config = LogKVConfig(vocab_size=10, d_model=512, num_heads=8, d_ff=1024,
        num_layers=NUM_LAYERS, conv_kernel_size=4, learnable_decay=True, chunk_size=4, phase_emb=False, phase_levels=2,
    ...
    assert extra and all(key.endswith('.level_decay') for key in extra)
    ...
    record = dict(passed=False, tasks=list(MODES), initial_weights_identical=True, learnable_decay=True,
        num_params=NUM_PARAMS, num_layers=NUM_LAYERS, conv_kernel_size=4, shared_two_layer_initial_weights_identical=True,
        base_initial_sha256=sha(BASE_INITIAL), extra_decay_parameters=sum(v.numel() for v in fresh_extra.values()), initial_weights_path=str(INITIAL),

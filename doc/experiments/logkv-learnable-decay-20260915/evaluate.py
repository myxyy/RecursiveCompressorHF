"""Frozen main task evaluator with observations from the actual generated samples."""
import argparse
...
from common import (MODES, ROOT, SOURCE, NUM_LAYERS, bind_task, name, run_dir, save, task_identity)
TASK_NAME = os.environ["LOGKV_TASK"]

sys.path.insert(0, str(SOURCE))
sys.path.insert(0, str(SOURCE / 'exp/copying'))
task = bind_task(TASK_NAME)
...
def main():
    ...
    assert args.mode == TASK_NAME
    if args.smoke: ev.build_t_grid = lambda exponent: [3, 16, 65]
    out = ROOT / args.mode
    records = {}
    current = {}
    pending = None
    ...
    try:
        ...
        sys.argv = [str(SOURCE / 'exp' / TASK_NAME / 'evaluate.py'),
                    '--run-name', name(args.mode), '--samples', '256', '--max-t-exp', '17',

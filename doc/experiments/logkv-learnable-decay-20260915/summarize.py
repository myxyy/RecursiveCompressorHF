...
import math
import shutil
from common import HERE, ROOT, SOURCE, MODES, NUM_LAYERS, NUM_PARAMS, run_dir, save, sha
...
def main():
    ...
    # Independently replay the frozen task generator with the exact standard
    # evaluator RNG stream and minibatch sizes; do not trust task metadata alone.
    torch.set_num_threads(1)
    replay = {}
    for mode in MODES:
        spec=importlib.util.spec_from_file_location('audit_'+mode,SOURCE/'exp'/mode/'task.py')
        task=importlib.util.module_from_spec(spec); spec.loader.exec_module(task)
        assert task.TASK_NAME==mode
    ...
    def audit(mode,cp,kind,metrics,records,ts,n):
        ...
        for T in ts:
            ...
            key=(mode,kind,T); assert targets.setdefault(key,target)==target
            ...
            rows.append(dict(mode=mode,task=mode,position_mode="none",num_layers=NUM_LAYERS, conv_kernel_size=4,learnable_decay=True,checkpoint=cp,evaluation=kind,T=T,n=n,
    ...
    for mode in MODES:
        ...
        assert init['passed'] and init['deterministic_algorithms'] and init['task']==mode
        assert init['task_sha256']==sha(SOURCE/'exp'/mode/'task.py')
        assert init['num_layers'] == NUM_LAYERS and init['num_params'] == NUM_PARAMS
        assert init['shared_two_layer_initial_weights_identical']
        ...
        for cp,folder in [('best','model_best'),('final','model')]:
            ...
            assert cfg['num_layers'] == NUM_LAYERS and cfg['conv_kernel_size'] == 4
            assert cfg['self_slot'] and cfg['gated_attention'] and not cfg['phase_emb']
            assert cfg['learnable_decay'] and not cfg['kv_norm'] and not cfg['level_amplify']
            assert not cfg['v_norm_only']
            standard=json.loads((out/f'results_{cp}.json').read_text())
            assert standard['task']==mode
            assert standard['task_sha256']==sha(SOURCE/'exp'/mode/'task.py')
        ...
        for f in ['train_log.jsonl','run_config.json','best.json','initialization_audit.json','beta_log.jsonl']: shutil.copy2(run_dir(mode)/f,local/f)
    from coefficient_audit import main as coefficient_audit
    coefficient_audit()
    save(dest/'metrics.json',rows)
    with (dest/'metrics.csv').open('w') as f:
        writer=csv.DictWriter(f,fieldnames=rows[0].keys(),lineterminator='\n');writer.writeheader();writer.writerows(rows)
    table=['# Two-layer CausalConv LogKV with learnable level decay: automatically audited results','',
    ...
    for r in rows:
        if r['evaluation']=='standard' and r['T'] in [3,16,64,128,256,1024,8192,131072]:
    ...
    from compare_baseline import main as compare_baseline
    compare_baseline()
    save(dest/'review.json',dict(passed=True,tasks=list(MODES),standard_cells=2*2*41,scatter_positions_replayed=True,
        ...
        limitations='Single training seed; Memories/positions match best and final for each task/horizon; the two tasks have different data streams. No additional runs or 16M extension.'))

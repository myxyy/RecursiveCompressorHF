...
REPO = HERE.parents[2]
ROOT = Path('/mnt/raid0/RecursiveCompressor/experiments/logkv-learnable-decay-20260915')
BASE_ROOT = Path('/mnt/raid0/RecursiveCompressor/experiments/logkv-causal-conv-20260914')
SOURCE = ROOT / 'source'
...
def name(mode):
    ...
    return 'causal-conv4-learnable-decay-fixed10-20260915'
...
def command(mode, python):
    return [python, str(HERE / 'train_entry.py'), '--run-name', name(mode),
        '--arch', 'logkv', '--learnable-decay', '--gated-attention', '--self-slot', '--phase-levels', '2',

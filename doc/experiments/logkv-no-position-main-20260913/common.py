...
REPO = HERE.parents[2]
ROOT = Path('/mnt/raid0/RecursiveCompressor/experiments/logkv-no-position-main-20260913')
SOURCE = ROOT / 'source'
...
def name(mode):
    ...
    return 'no-position-fixed10-20260913'
...
def command(mode, python):
    return [python, str(HERE / 'train_entry.py'), '--run-name', name(mode),
        ...
        '--d-model', '512', '--num-heads', '8', '--d-ff', '1024', '--num-layers', '2',

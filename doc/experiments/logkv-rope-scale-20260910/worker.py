...
def main():
    ...
    record=dict(mode=a.mode,alpha=MODES[a.mode],started=now(),state='running',commands=[])
    ...
    try:
        ...
        old=json.loads(OLD_CONFIG.read_text()); ignore={'run_name','retrieval_rope_scale'}
        assert {k:v for k,v in cfg.items() if k not in ignore}=={k:v for k,v in old.items() if k not in ignore}
        assert cfg['retrieval_rope_scale']==MODES[a.mode]

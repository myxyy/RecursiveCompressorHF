...
- **Hidden format**: `(levels, offset)`; `levels[i] = [cur_q, cur_k, cur_v, prev_k, prev_v]` with heads folded into the batch dim; offset is required to reconstruct absolute positions (phase, window bases). O(C·log L·d) memory.

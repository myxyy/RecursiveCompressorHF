...
- **Optimizers**: Muon (`torch.optim.Muon`) for 2D hidden Linear weights with `adjust_lr_fn="match_rms_adamw"`; AdamW for embedding, head, learnable contexts (`compressor_query`, `initial_context`), biases, and RMSNorms. Both share the same LR. `split_params_for_muon()` does the partition by `param.ndim >= 2`, so 1D RMSNorm weights naturally fall on AdamW.
